 ...
 * 
 */
require_once(EGW_INCLUDE_ROOT. '/spisession/inc/class.reg_status_so.inc.php');	

class reg_status_bo extends reg_status_so{
	
	/**
	 * Constructeur 
	 *
	 */
	function reg_status_bo(){
		parent::reg_status_so();
	}
	 ...
	 * @return array
	 */
		$info = $this->reg_so_status->read($id);

		$transition = $this->reg_so_transition_status->search(array('status_source' => $id),false);
	 ...
	 */
		...
		$rows = $this->reg_so_status->search($search,false,$order,'',$wildcard,false,$op,$start,$query['col_filter']);
		if(!$rows){
			$rows = array();
		}
		foreach((array)$rows as $id=>$value){
			// Récupération des transition
			$transition = $this->reg_so_transition_status->search(array('status_source' => $value['status_id']),false);
			foreach((array)$transition as $keyTransition => $dataTransition){
		...
		return $this->reg_so_status->total;	
    }
	 ...
	 */
		$retour = array();
		$info = $this->reg_so_status->search(array('status_active' => true),false,'status_label');
		$i = 0;
