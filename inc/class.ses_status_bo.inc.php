 ...
 * 
 */
require_once(EGW_INCLUDE_ROOT. '/spisession/inc/class.ses_status_so.inc.php');	

class ses_status_bo extends ses_status_so{
	
	/**
	 * Constructeur 
	 *
	 */
	function ses_status_bo(){
		parent::ses_status_so();
	}
	 ...
	 * @return array
	 */
		$info = $this->ses_so_status->read($id);

		// Recuperation des transitions
		$transition = $this->ses_so_transition_status->search(array('status_source' => $id),false);
	 ...
	 */
		...
		$rows = $this->ses_so_status->search($search,false,$order,'',$wildcard,false,$op,$start,$query['col_filter']);
		if(!$rows){
			$rows = array();
		}
		foreach((array)$rows as $id=>$value){
			// Recuperation des transitions
			$transition = $this->ses_so_transition_status->search(array('status_source' => $value['status_id']),false);
			foreach((array)$transition as $keyTransition => $dataTransition){
		...
		if($query['filter']){
			$GLOBALS['egw_info']['flags']['app_header'] .= ' - '.lang("Search status '%1'",$this->id2name($query['filter']));
		}
		return $this->ses_so_status->total;	
    }
	 ...
	 */
		$retour = array();
		$info = $this->ses_so_status->search(array('status_active' => true),false,'status_label');
		$i = 0;
