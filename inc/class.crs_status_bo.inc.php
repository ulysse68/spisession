 ...
 * 
 */
require_once(EGW_INCLUDE_ROOT. '/spisession/inc/class.crs_status_so.inc.php');	

class crs_status_bo extends crs_status_so{
	
	/**
	 * Constructeur 
	 *
	 */
	function crs_status_bo(){
		parent::crs_status_so();
	}
	 ...
	 * @return array
	 */
		$info = $this->crs_so_status->read($id);

		$transition = $this->crs_so_transition_status->search(array('status_source' => $id),false);
	 ...
	 */
		...
		$rows = $this->crs_so_status->search($search,false,$order,'',$wildcard,false,$op,$start,$query['col_filter']);
		if(!$rows){
			$rows = array();
		}
		foreach((array)$rows as $id=>$value){
			// Récupération des transitions
			$transition = $this->crs_so_transition_status->search(array('status_source' => $value['status_id']),false);
			foreach((array)$transition as $keyTransition => $dataTransition){
		...
		return $this->crs_so_status->total;	
    }
	 ...
	 */
		$retour = array();
		$info = $this->crs_so_status->search(array('status_active' => true),false,'status_label');
		$i = 0;
