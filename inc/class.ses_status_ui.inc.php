 ...
 * 
 */
require_once(EGW_INCLUDE_ROOT. '/spisession/inc/class.ses_status_bo.inc.php');


class ses_status_ui extends ses_status_bo{
	 ...
	 *
	 */
	function ses_status_ui(){
		parent::ses_status_bo();
	 ...
	 */
		...
		if (!is_array($content['nm']))
		{
			...
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spisession.ses_status_bo.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
		...
		$tpl = new etemplate('spisession.ses_status.index');
		$content['nm']['header_right'] = 'spisession.ses_status.index.right';
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Status management');
		$tpl->read('spisession.ses_status.index');
		$tpl->exec('spisession.ses_status_ui.index', $content, $sel_options, $readonlys, array('nm' => $content['nm']));
	}
	 ...
	 */
		...
		$tpl->exec('spisession.ses_status_ui.edit', $content, $sel_options, $readonlys, $content,2);
	}
}
?>