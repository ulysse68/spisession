 ...
 * 
 */
require_once(EGW_INCLUDE_ROOT. '/spisession/inc/class.date_status_bo.inc.php');


class date_status_ui extends date_status_bo{
	 ...
	 *
	 */
	function date_status_ui(){
		parent::date_status_bo();
	 ...
	 */
		...
		if (!is_array($content['nm']))
		{
			...
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spisession.date_status_bo.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
		...
		$tpl = new etemplate('spisession.date_status.index');
		$content['nm']['header_right'] = 'spisession.date_status.index.right';
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Management of the dates\' status');
		$tpl->read('spisession.date_status.index');
		$tpl->exec('spisession.date_status_ui.index', $content, $sel_options, $readonlys, array('nm' => $content['nm']));
	}
	 ...
	 */
		...
		$tpl->exec('spisession.date_status_ui.edit', $content, $sel_options, $readonlys, $content,2);
	}
}
?>